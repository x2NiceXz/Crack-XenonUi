local library = {
	Version = "0.0.1b",
	WorkspaceName = "XYz Hub Premium Script",
	flags = {},
	signals = {},
	objects = {},
	elements = {},
				...
				if getsynasset then
					...
					elseif #image > 14 and string.sub(image, 1, 14) == "synasseturl://" then
						local x, e = pcall(function()
							...
							local fold = isfolder("./Xenon Hub Premium Scripts")
							if not fold then
								makefolder("./XYz Hub Premium Scripts")
							end
							fold = isfolder("./XYz Hub Premium Scripts/Themes")
							if not fold then
								makefolder("./XYz Hub Premium Scripts/Themes")
							end
							fold = isfolder("./XYz Hub Premium Scripts/Themes/SynapseAssetsCache")
							if not fold then
								makefolder("./XYz Hub Premium Scripts Themes/SynapseAssetsCache")
							end
							if not fold or not isfile("./Xenon Hub Premium Scripts/Themes/SynapseAssetsCache/" .. codename .. ".dat") then
								local res = game:HttpGet(string.sub(image, 15))
								if res ~= nil then
									writefile("./XYz Hub Premium Scripts/Themes/SynapseAssetsCache/" .. codename .. ".dat", res)
								end
							end
							return getsynasset(readfile("./XYz Hub Premium Scripts/Themes/SynapseAssetsCache/" .. codename .. ".dat"))
						end)
						if x and e ~= nil then
							return e
						end
...
function library:CreateWindow(options, ...)
	...
	if windowName and #windowName > 0 and library.WorkspaceName == "XYz Hub Premium Scripts" then
		library.WorkspaceName = convertfilename(windowName, "XYz Hub Premium Scripts")
	end
